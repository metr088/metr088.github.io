...
$db_config_path = '../setting/database.php';
...
if($_POST) {
	...
	if($core->validate_post($_POST) == true)
	{
		if($database->create_database($_POST) == false) {
			...
			$message = $core->show_message('error',"Файл конфигурации БД не можеть быть записан, убедитесь что chmod setting/database.php равен 777");
		} else if ($core->write_scconf($_POST) == false) {
			$message = $core->show_message('error',"Файл конфигурации скрипта не можеть быть записан, убедитесь что chmod setting/config.php равен 777");
		}
...
?>
<div class="alert alert-danger">Не установлен ZipArchive</div>
<?
}else{
?>
				 ...
				 <?php if(isset($message)) {echo '<div class="alert alert-danger">' . $message . '</div>';}?>
				  <form id="install_form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
					...
					<fieldset>
					  ...
					  <div class="form-group"><label for="useremail">Логин</label><input type="text" id="useremail" class="form-control" name="useremail" /></div>
