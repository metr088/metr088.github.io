...
foreach ($all as $row) {
$f = $this->input->toJson($row['result']);
$onedate[date("d.m.Y", $f['time'])] = 0;
}
foreach ($all as $row) {
$f2 = $this->input->toJson($row['result']);
$onedate[date("d.m.Y", $f2['time'])] += 1;
}
...
$one = $this->db->get('manager', ['result'], ['user_id' => $this->session['user_id'],'ORDER' => ['id' => 'ASC']]);
$fa = $this->input->toJson($one['result']);
$from = date("d.m.Y", $fa['time']);
...
<div class="col-md-12 col-sm-12 col-xs-12">
          <div class="info-box">
            ...
            <div class="info-box-content">
              <span class="info-box-text">Сколько логов</span>
              <span class="info-box-number"><? echo $this->db->count('manager', ['user_id' => $this->session['user_id']]); ?></span>
            </div>
          </div>
</div>
...
$inform = $this->db->select('manager', ['result'], ['user_id' => $this->session['user_id']]);
